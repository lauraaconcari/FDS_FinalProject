...
# Leggi il dataset
dataset = pd.read_csv('playlists_KNN.csv')  # Sostituisci con il tuo percorso e nome del file CSV
...
for tag_value in tag_values:
    ...
    predicted_tag_data = dataset[dataset['predicted_Tag_KNN'] == tag_value]
    ...
    plt.bar(index, means_original, bar_width, label=f'TAG: {tag_value} - Original', alpha=0.7, color = 'orange')
    plt.bar([i + bar_width for i in index], means_predicted, bar_width, label=f'TAG: {tag_value} - Predicted', alpha=0.7, color='pink', align='edge')
